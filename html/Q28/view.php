...
<body>
  ...
  if ($_FILES['upfile']['error'] !== UPLOAD_ERR_OK) {
    ...
    if (!move_uploaded_file($src, 'up/' . $dest)) {
      ...
      $weeks = $week[$now -> format("w")];
      $weeks = "(".$weeks.") ";
      $date = $now -> format('Y年m月d日');
      $time = $now -> format('H:i:s');
      $nows = $date.$weeks.$time;
      $data = $nows . "," . $_POST["comment"] . "," . $_FILES['upfile']['name'] . "\n";
      fwrite($file, $data);
      flock($file, LOCK_UN);
      fclose($file);
    }
  ...
  flock($file, LOCK_SH);
  print "<table><tr><td width='250px'>日時</td><td width='200px'>コメント</td><td width='420px'>画像</td></tr>";
  while ($fline = fgets($file, 1024)) {
    //1行読み込んで、コンマごとに分解して配列に格納
    $line = explode(",", $fline);
    //テーブルタグで編集して表示
    printf("<tr><td>%s</td><td>%s</td><td><img src='up/%s' ></td></tr>", $line[0], $line[1],$line[2]);
  }
  print '</table>';
  flock($file, LOCK_UN);
  fclose($file);
